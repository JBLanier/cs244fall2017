...
const int MAX_SAMPLE_TRIPLET_SIZE = 81; // max size example: {"t":1508735981516,"ir":262143,"r":262143,"x":-23.0026,"y":-23.0026,"z":-23.0026}
...
//   // We use bignumber to handle full-size timestamps
  ...
  myIMU.fifoBegin();
  uint8_t returnData = myIMU.begin();
  if (( returnData != 0x00) && (returnData != 0xFF)) {
    while(1) {
      Serial.println("Problem starting Accelerometer.");
      delay(1000);
    }
  }
...
void loop() {
  ...
  int accel_str_len = 7;
  char accel_str[accel_str_len];
  dtostrf(x, accel_str_len, 4, accel_str);  
  json_offset += sprintf(json_str_at_offset, "\"x\":%s,", accel_str);
  json_str_at_offset = &json_str[json_offset];  
  dtostrf(y, accel_str_len, 4, accel_str);  
  json_offset += sprintf(json_str_at_offset, "\"y\":%s,", accel_str);
  json_str_at_offset = &json_str[json_offset];  
  dtostrf(z, accel_str_len, 4, accel_str);  
  json_offset += sprintf(json_str_at_offset, "\"z\":%s},", accel_str);
  ...
  if (samples_taken >= NUM_SAMPLES_PER_POST) {
    ...
    //Serial.println("");
    doBetterPost("/ppg_accel_samples",json_str, json_offset);
    resetJsonString();
    samples_taken = 0;
  }
